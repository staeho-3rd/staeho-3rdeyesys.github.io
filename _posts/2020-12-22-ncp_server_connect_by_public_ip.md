...
네이버 클라우드에서는 포트 포워딩과 공인 IP가 동시에 설정되어 있을 경우 포트 포워딩이 우선 적용됩니다.  
즉, **포트 포워딩이 설정된 상태에서는 Putty로 접속할 때 공인IP로는 접속 되지 않는다**는 뜻입니다.  
그러므로, 서버 접속을 공인 IP로 하는 경우에는 포트 포워딩을 해제하시고,  
